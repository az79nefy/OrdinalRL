...
# Discount factor
gamma = 0.9
# Lambda controls decay of eligibility trace after each action
lambda_ = 0.7
...
# Eligibility Trace (2-dimensional array with float-value for each action in each observation)
eligibility_trace = np.array([[0.0 for x in range(n_actions)] for y in range(n_observations)])
...
for i_episode in range(n_episodes):
    ...
    # reset eligibility trace every episode
    eligibility_trace *= 0
